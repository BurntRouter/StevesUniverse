package com.router.stevesuniverse.entities;
...
public class Amethyst_Whip_Entity extends EntityThrowable
{
	...
	private Vec3i swingVec = null;

	/** The y-motion factor, based on distance from rotation point */
	private double dy;

	public Amethyst_Whip_Entity(World world) {
		super(world);
	}

	public Amethyst_Whip_Entity(World world, EntityLivingBase entity) {
		super(world, entity);
	}

	public Amethyst_Whip_Entity(World world, double x, double y, double z) {
		super(world, x, y, z);
	}
	...
	public void setThrower(EntityPlayer player) {
		dataWatcher.updateObject(THROWER_INDEX, player != null ? player.getCommandSenderEntity() : "");
	}

	@Override
	public EntityLivingBase getThrower() {
		String name = dataWatcher.getWatchableObjectString(THROWER_INDEX);
		return (name.equals("") ? null : world.getPlayerEntityByName(name));
	}
